...
// The frontend currently reads `NEXT_PUBLIC_BACKEND_URL` at build time
...
// backend's actual runtime origin through
// `webPreferences.additionalArguments` when it creates the `BrowserWindow`
// (see main.ts) - Electron appends those strings onto this preload
// script's own `process.argv`, letting this script read the value back
// out below before any of the page's own scripts run.
...
// Exposed unconditionally (unlike `__BACKEND_URL__` above) so the
// frontend's Settings page can detect it's running inside the desktop app
// at all - each method forwards to an `ipcMain.handle` registered in
// main.ts.
